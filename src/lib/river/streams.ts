import type { AsyncIterableStream, Tool, ToolSet } from 'ai';
import { stepCountIs, streamText, tool } from 'ai';
import { createRiverStream, defaultRiverProvider, RiverError } from '@davis7dotsh/river-core';
import { z } from 'zod';
...
type ExtractStreamChunkType<T> = T extends AsyncIterableStream<infer U> ? U : never;

type QuestionAskerChunkType = ExtractStreamChunkType<ReturnType<typeof questionAskerAgent>>;
...
>()
	...
	.runner(async ({ input, stream, adapterRequest }) => {
		const { question } = input;
		const { appendChunk, close, sendFatalErrorAndClose } = stream;

		const { isAuthenticated } = await checkAuthStatus(adapterRequest.event);

		if (!isAuthenticated) {
			sendFatalErrorAndClose(new RiverError('You are not authenticated', undefined, 'custom'));
		}
